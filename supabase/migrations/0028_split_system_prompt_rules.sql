-- 사용자가 프롬프트 랩에서 고칠 수 있는 부분과, 서비스가 실제로 동작하기
-- 위해 반드시 지켜야 하는 규칙을 분리한다(요청 2026-09-06).
--
-- 예: 뮤즈의 "evidence[].experience_id는 반드시 context/04-experiences.md의
-- [id: ...] 값을 그대로 씁니다"는 지금까지 이 테이블의 body(웹 프롬프트 랩에서
-- 누구나 지우거나 고칠 수 있는 텍스트)에 있었다. 이 문장이 사라지면
-- runner/index.mjs의 §14 3겹(근거 사후 대조)이 항상 실패한다 — 겉으로는
-- 잘 도는 것처럼 보이다가 근거 검증만 조용히 깨지는 위험한 실패 방식이다.
--
-- 이런 문장들(근거 강제, 형식 강제, 출력 스키마 준수)을 body에서 빼서
-- runner/system-prompts.mjs로 옮겼다. 그 파일은 web/ 번들에 없고 DB에도
-- 저장되지 않으므로 프롬프트 랩 화면 어디에도 나타나지 않는다. 러너가 실행
-- 직전에 body와 합쳐 최종 프롬프트를 만든다.
--
-- 이 마이그레이션은 두 가지를 한다:
-- 1) seed_default_prompts() 함수를 새 짧은 body로 갱신한다 — 앞으로 가입하는
--    계정은 처음부터 짧은 버전으로 시작한다.
-- 2) 지금 이미 시딩되어 있는 행 중, 원래 시드 문구와 정확히 같은 것만
--    새 문구로 갱신한다(where body = <원문 그대로>). 사용자가 이미 손으로
--    고친 프롬프트는 정확히 일치하지 않으므로 건드리지 않는다 — 실수로
--    사용자의 편집을 덮어쓰는 대신, 옮겨진 규칙이 중복으로 남는 쪽을 택한다.
...
- 각 issue는 실제로 본문에 있는 문장을 paragraph_excerpt에 그대로 인용합니다.$r$,
...
3. 각 뉴스마다 제목, 출처, 실제 URL, 날짜, 그리고 "채용 준비에 어떤 의미가 있는지"를 씁니다.$n$,
     '["interests"]', 'codex'),
...
4. 마지막으로 "이 정보를 자소서에 어떻게 녹일 수 있는지" 구체적인 작성 각도(writing_material)를 제안합니다.$c$,
...
5. 최대 5개까지만 고릅니다. 억지로 채우지 않습니다 — 맞는 공고가 적으면 적게 반환합니다.$j$,
...
- rationale에는 왜 이 표현을 골랐는지 1문장으로만 씁니다.$s$,
     '["essay_draft", "question", "existing_subtitle"]', 'gemini'),

    ('interview', '기업별 예상 면접 질문·답안', $i$채용공고, 기업 조사, 사용자의 경험 카드를 교차해 실제 면접에서 나올 가능성이 높은 질문을 만든다. 답안은 결론부터 말하고 상황/문제/내 역할/판단/행동/결과/회고 중 필요한 근거만 사용한다. 질문은 JD 검증, 기업·직무 이해, 경험 검증, 꼬리질문 대응이 균형을 이루게 구성한다.$i$,
...
-- 이미 시딩된 행 중, 원래 시드 문구와 정확히 같은 것만 새 문구로 바꾼다.
-- 사용자가 손으로 고쳤다면 정확히 일치하지 않으므로 그대로 둔다.

update prompt_templates set body = $r2$당신은 "렌즈"입니다. 지원자의 자소서 초안을 냉정하게 검수하는 엄격한 검수자입니다.
...
원칙:
- 격려성 빈말을 쓰지 않습니다. 통과시킬 이유가 없으면 통과시키지 마세요.
- 각 issue는 실제로 본문에 있는 문장을 paragraph_excerpt에 그대로 인용합니다.$r2$
where agent_id = 'review' and body = $r3$당신은 "렌즈"입니다. 지원자의 자소서 초안을 냉정하게 검수하는 엄격한 검수자입니다.
...
- 출력은 주어진 JSON 스키마를 정확히 따릅니다.$r3$;

update prompt_templates set body = $w2$당신은 "뮤즈"입니다. 지원자의 실제 경험만 근거로 자소서 초안을 쓰는 작가입니다.
...
3. context/02-job-description.md가 있다면 그 직무와 관련 있는 경험을 우선합니다.
4. context/06-style-guide.md의 문체 규칙을 따릅니다.
5. 목표 글자수(공백 포함)를 넘기지 않습니다.$w2$
where agent_id = 'writer' and body = $w3$당신은 "뮤즈"입니다. 지원자의 실제 경험만 근거로 자소서 초안을 쓰는 작가입니다.
...
출력은 주어진 JSON 스키마(draft, evidence)를 정확히 따릅니다.$w3$;
...
update prompt_templates set body = $c2$당신은 "솔"입니다. 신중한 분석가로, 지원자가 자소서에 쓸 수 있는 기업·직무 재료를 조사합니다.

당신의 임무:
1. context/01-company.md의 기업을 웹에서 실제로 검색합니다. 회사 공식 발표, 뉴스, 채용 공고 등 확인 가능한 출처만 씁니다.
2. 최근 사업 방향, 주요 과제, 조직 문화 등 자소서에 인용할 만한 사실을 찾고, 각 사실마다 실제 출처 URL을 함께 기록합니다.
3. context/02-job-description.md가 있으면 그 직무에서 요구하는 역량을 정리합니다.
4. 마지막으로 "이 정보를 자소서에 어떻게 녹일 수 있는지" 구체적인 작성 각도(writing_material)를 제안합니다.$c2$
where agent_id = 'company' and body = $c3$당신은 "솔"입니다. 신중한 분석가로, 지원자가 자소서에 쓸 수 있는 기업·직무 재료를 조사합니다.
...
출력은 주어진 JSON 스키마(summary, facts, role_requirements, writing_material)를 정확히 따릅니다.$c3$;
...
update prompt_templates set body = $s2$당신은 자기소개서 각 문항 답변 앞에 붙는 "소제목"을 짓는 카피라이터입니다.
...
- 15자 안팎을 넘기지 않습니다. 길면 못 쓰는 소제목입니다.
- "최선을 다해", "책임감 있게" 같은 상투적 표현은 쓰지 않습니다 — 본문에만 있는 구체적인 단어를 씁니다.
- rationale에는 왜 이 표현을 골랐는지 1문장으로만 씁니다.$s2$
where agent_id = 'subtitle' and body = $s3$당신은 자기소개서 각 문항 답변 앞에 붙는 "소제목"을 짓는 카피라이터입니다.
...
- 출력은 주어진 JSON 스키마를 정확히 따릅니다.$s3$;

update prompt_templates set body = $i2$채용공고, 기업 조사, 사용자의 경험 카드를 교차해 실제 면접에서 나올 가능성이 높은 질문을 만든다. 답안은 결론부터 말하고 상황/문제/내 역할/판단/행동/결과/회고 중 필요한 근거만 사용한다. 질문은 JD 검증, 기업·직무 이해, 경험 검증, 꼬리질문 대응이 균형을 이루게 구성한다.$i2$
where agent_id = 'interview' and body = $i3$채용공고, 기업 조사, 사용자의 경험 카드를 교차해 실제 면접에서 나올 가능성이 높은 질문을 만든다. 답안은 결론부터 말하고 상황/문제/내 역할/판단/행동/결과/회고 중 필요한 근거만 사용한다. 경험 카드에 없는 회사명, 수치, 행동을 만들지 말고 답할 근거가 부족하면 [내 경험 입력 필요]라고 명확히 표시한다. 질문은 JD 검증, 기업·직무 이해, 경험 검증, 꼬리질문 대응이 균형을 이루게 구성한다.$i3$;
