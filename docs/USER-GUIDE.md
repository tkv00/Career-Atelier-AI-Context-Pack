...
브라우저에서 http://localhost:3000 접속 → 본인 이메일 입력 → 메일의 매직링크 클릭.

> **가장 먼저 가입한 계정이 이 인스턴스의 소유자**가 되고, 그 뒤로는 아무도 가입할 수 없습니다. 첫 로그인을 본인 이메일로 하세요.
...
러너가 켜져 있는 동안 **6시간마다** 전체 데이터를 JSON으로 그 폴더에 저장합니다. 하루에 파일 하나라 폴더가 무한정 커지지 않습니다.
