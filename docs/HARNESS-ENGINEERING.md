...
웹에서 올린 PDF·Word·PowerPoint·Excel 원본은 모델 작업 폴더에 직접 넣지 않는다. 로컬 러너가 격리된 Python 가상환경의 Microsoft MarkItDown 0.1.7 `convert_local()`로 Markdown을 만든 뒤, 크기 제한을 다시 적용한 Markdown 또는 검증된 표 조각만 컨텍스트로 전달한다. Excel은 기존 열 매핑·병합 셀·저장된 수식 결과 검증을 유지한다.

학력에 `성적증명서` 종류의 PDF가 첨부되면 `transcript` 작업을 자동으로 큐에 넣는다. 러너는 원본 PDF를 모델 워크스페이스 밖에서 Markdown으로 변환하고 즉시 임시 원본을 지운다. 구조화 출력의 과목명·학기·성적·세부 정보는 반드시 Markdown의 연속 인용으로 대조하며, 같은 학력에 이미 있는 과목 키(과목명·학기·학점·성적)는 다시 저장하지 않는다. 이미지뿐인 스캔 PDF를 외부 OCR API로 보내는 우회는 하지 않는다.
