...
[처음 설치하는 분은 README 순서대로 진행하세요.](../README.ko.md)
...
2. **나를 알려 주기:** 관제실에서 목표 직무·관심 분야를 입력하고, 이력 정보에 학력·경력 등을 저장합니다. 경험 카드에는 실제 상황·판단·행동·결과를 정리합니다. 기존 파일은 [MCP로 가져올 수 있습니다](#mcp로-정리본-일괄-가져오기).
3. **비서 배정:** 프롬프트에서 각 비서가 사용할 AI를 로그인해 둔 CLI로 지정합니다. 원하는 문체와 작성 기준도 저장합니다.
...
개인정보, 비용, AI 선택, 백업, 로그인, 가져오기는 [별도 FAQ](FAQ.ko.md)에 정리했습니다.
...
질문은 카테고리(기업·직무·경험 등)로 분류되어 저장되고, 면접 준비 화면에서 답변을 적고 다듬을 수 있습니다.
...
> **각 비서가 어떤 LLM으로 돌지는 [프롬프트](#주요-화면)에서 바꿀 수 있습니다.** 위 표는 기본값입니다.
>
...
<!-- 자리표시자 — 이 파일을 실제 캡처로 덮어쓰세요. 규격: docs/images/screens/README.md -->
<img src="../docs/images/screens/02-calendar.png" alt="지원 일정" width="100%">
...
<!-- 자리표시자 — 이 파일을 실제 캡처로 덮어쓰세요. 규격: docs/images/screens/README.md -->
<img src="../docs/images/screens/04-prompt-lab.png" alt="프롬프트" width="100%">

<br>

### 이력 정보

학력·자격증·대외활동·교육활동·프로젝트·경력사항·수상내역, 7개 섹션을 항목별로 기록합니다. 사이드바의 **"이력 정보"**에서 들어갑니다.

지원서를 쓸 때마다 학점이나 자격증 등록번호를 다시 찾지 않아도 됩니다. 성적증명서·졸업증명서 같은 파일도 함께 보관하며, 파일은 비공개 저장소에 올라가고 열람할 때만 60초짜리 링크가 만들어집니다.

<!-- 자리표시자 — 이 파일을 실제 캡처로 덮어쓰세요. 규격: docs/images/screens/README.md -->
<img src="../docs/images/screens/07-records.png?v=0750026" alt="이력 정보" width="100%">
...
<!-- 자리표시자 — 이 파일을 실제 캡처로 덮어쓰세요. 규격: docs/images/screens/README.md -->
<img src="../docs/images/screens/05-experiences.png?v=0750026" alt="경험 카드" width="100%">
...
<!-- 자리표시자 — 이 파일을 실제 캡처로 덮어쓰세요. 규격: docs/images/screens/README.md -->
<img src="../docs/images/screens/06-interviews.png?v=0750026" alt="면접 준비" width="100%">
...
**셋 다 설치할 필요는 없습니다.** 이용 가능한 계정의 CLI 하나를 설치하고, 프롬프트에서 사용할 비서들을 그 제공자로 지정하세요. 로그인하지 않은 CLI로 요청한 작업은 실행할 수 없습니다.
...
설치의 기준 문서는 [README의 시작하기](../README.ko.md#시작하기)입니다. 준비물 확인 → Supabase 연결 → 웹 실행·첫 가입 → 선택적 AI 연결 순서로 진행하세요. 각 단계의 실행 위치와 완료 기준을 함께 안내합니다.

### AI에게 설치 맡기기

[README의 설치 안내](../README.ko.md#시작하기) 또는 [에이전트 설치 지침](AI-INSTALL.md)을 사용하세요. Supabase 로그인, 첫 가입, 기기 승인은 본인이 진행합니다.

### 실행

[웹·Runner 실행 순서](../README.ko.md#시작하기)를 따르세요. 웹에서 먼저 가입한 뒤 Runner에 같은 계정으로 로그인합니다. [로그인 문제 해결](AUTH-TROUBLESHOOTING.md)
...
**엑셀이나 Notion에 쌓아 둔 내 경험을 다시 입력하지 마세요.** MCP(Model Context Protocol)는 AI 클라이언트가 외부 도구를 호출하는 공통 연결 규약입니다. 이 저장소의 `career-atelier` 서버는 Notion 페이지·데이터 소스 또는 로컬 Excel(`.xlsx`)·Markdown·JSON 파일을 읽어 Career Atelier의 이력정보와 경험 카드로 가져옵니다. 웹 화면의 버튼이 아니라 내 PC의 MCP 클라이언트나 터미널에서 실행합니다.

### 어떤 일을 맡길 수 있나요?

| 도구 | 역할 | 주요 인자 |
|---|---|---|
| `preview_import` | 인식된 항목·저장 예정 테이블·경고 확인. DB에 쓰지 않음 | `source`, `section`, 선택적 `sheet`, `column_map` |
| `import_records` | 항목 추가 또는 기존 항목 갱신 | 미리보기 인자, `dry_run`, 선택적 `only`, `expected_digest` |
...
서버는 소스를 로컬에서 파싱해 DB에 기록합니다. 원문 전체를 모델에게 전달해 INSERT 인자로 다시 작성시키지 않고, 클라이언트에는 경로와 건수·제목·결과 요약을 주고받습니다. 구조화 작업 자체에는 LLM 호출이 없습니다. 재현 실험은 직접 호출·MCP 호출의 실제 처리 시간과 원문 중계 방식의 가정 기준선을 분리합니다. 문자 기반 토큰 추정치는 실제 제공자 사용량이나 절감 보장값이 아닙니다. `cd runner` 후 `npm run mcp:research`로 로컬 벤치마크 결과를 생성할 수 있습니다.
...
엑셀은 첫 행을 열 제목으로 두고 이후 한 행에 경험 하나를 작성합니다. [합성 예제 엑셀](../runner/mcp/fixtures/sample-experiences.xlsx)의 내용을 실제 경험으로 바꾸고 필드 제목은 유지하면 됩니다. 시트와 기록 종류를 선택하고, 기존 열 이름은 `column_map`으로 지원 필드에 연결할 수 있습니다.
...
예를 들어 `column_map: {"경험 이름":"title","성과 요약":"result"}`로 열을 연결합니다. 인식하지 못한 엑셀 열은 경고에 표시되며 값은 저장되지 않습니다. 수식·병합 셀은 거부하므로 일반 표의 값으로 내보내세요. 구형 `.xls`, CSV, OCR, 자유 서술의 자동 추출은 지원하지 않습니다.
...
미리보기의 `source_digest`를 저장 요청의 `expected_digest`로 전달하면 그사이 소스나 매핑이 바뀐 경우 저장을 거부합니다. 선택적 검사이므로 저장 전에 진단 결과도 확인하세요. 각 진단 목록과 제목 예시는 20개까지 반환하고, 전체 진단 건수는 `diagnostic_counts`에 남깁니다.
...
3. 클라이언트의 MCP 서버를 다시 시작합니다. 실행 위치와 관계없이 `runner/.env`를 자동으로 읽으며, 기존 프로세스 환경변수가 우선합니다. 파일용 등록 명령을 그대로 사용하면 됩니다.

| 소스 | 전달할 값 | 문서 구조 |
|---|---|---|
| 페이지 | `source: "notion://page/<페이지-ID>"` | 본문의 제목 1로 종류 지정 |
| 데이터베이스 | `source: "notion://database/<DB-ID>", section: "경험"` | 행 제목이 항목명, 속성명이 필드명 |
| 데이터 소스 | `source: "notion://data-source/<소스-ID>", section: "경험"` | 여러 소스가 있는 DB에서 하나를 지정 |

공유 링크에서 페이지·DB의 ID를 확인하고 URL 전체나 `?v=` 뒤의 보기 ID 대신 전달합니다. DB는 한 번에 한 종류로 가져오며, 속성 이름을 위 필드 형식에 맞춥니다. DB 행의 본문과 첨부파일은 가져오지 않습니다.

어댑터는 Notion API `2025-09-03`의 데이터 소스 조회와 페이지네이션을 사용합니다. 소스가 하나인 DB는 자동 선택하며, 여러 개면 오류에 안내된 ID로 하나를 지정합니다. 사용자 정의 속성 이름은 `column_map`으로 연결하고, 지원하지 않는 속성은 경고로 확인합니다.
...
같은 인자로 `import_records`를 호출하면서 `dry_run: false`를 추가하면 저장합니다. 페이지와 DB 모두 터미널에서 미리보기할 수 있습니다.

```bash
cd runner
node mcp/server.mjs preview --source "notion://page/<페이지-ID>"
...
| `NOTION_TOKEN` 없음 | 로컬 `runner/.env`, 우선 적용되는 프로세스 변수, MCP 서버 재시작 |
...
**이번 확장의 검증 범위:** 실제 XLSX 파싱, 필드 비교, stdio 호출과 480회 시간 측정을 실행했습니다. Notion은 고정 HTTP 응답으로, 저장·갱신·일부 실패는 메모리 DB 대역으로 테스트했습니다. 이번 실험은 실 Notion 계정 호출, 운영 Supabase 쓰기, AI 제공자 토큰 측정을 포함하지 않습니다. [Notion 버전 변경 안내](https://developers.notion.com/guides/get-started/upgrade-guide-2025-09-03) · [MCP 구현과 상세 검증 기록](../runner/mcp/README.md)
...
| 추가 대상 | 학력·수강 과목·자격증·대외활동·교육·프로젝트·경력·수상, 첨부 메타데이터, 수정 요청·제안 |
| 제외 | Storage 파일 원본, 인증 정보, 작업 큐, 러너, 상세 실행 이벤트, 가져오기 캐시 |
| 대용량 | ID순 페이지 조회 후 완성된 임시 파일로 당일 파일을 교체. 내보내기 중 수정까지 시점 일치를 보장하는 DB 스냅샷은 아님 |
