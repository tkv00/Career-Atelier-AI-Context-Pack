...
서버는 소스를 로컬에서 파싱해 DB에 기록합니다. 원문 전체를 모델에게 전달해 INSERT 인자로 다시 작성시키지 않고, 클라이언트에는 경로와 건수·제목·결과 요약을 주고받습니다. 구조화 작업 자체에는 LLM 호출이 없습니다. 재현 실험은 직접 호출·MCP 호출의 실제 처리 시간과 원문 중계 방식의 가정 기준선을 분리합니다. 문자 기반 토큰 추정치는 실제 제공자 사용량이나 절감 보장값이 아닙니다. `cd runner` 후 `npm run mcp:research`로 실행하고, [한국어 연구 보고서](../docs/research/mcp-import/REPORT.ko.md)와 [개별 SVG/PDF 시각자료](../docs/research/mcp-import/index.html)를 확인하세요.
...
엑셀은 첫 행을 열 제목으로 두고 이후 한 행에 경험 하나를 작성합니다. [합성 예제 엑셀](../docs/research/mcp-import/examples/ko-2.xlsx)의 내용을 실제 경험으로 바꾸고 필드 제목은 유지하면 됩니다. 시트와 기록 종류를 선택하고, 기존 열 이름은 `column_map`으로 지원 필드에 연결할 수 있습니다.
