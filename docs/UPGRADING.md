...
4. 저장소 최상위에서 `npm start`를 실행합니다. 잠금 파일이 바뀌었으면 필요한 웹·Runner 패키지를 다시 설치하고, 마이그레이션 구성이 바뀌었으면 기존 설정의 프로젝트 ref로 원격 적용 이력을 검사한 뒤 새 파일만 적용합니다. Supabase CLI 로그인이 만료된 경우에만 브라우저 로그인을 요청합니다.
5. 릴리스 안내에 별도 수동 단계가 있으면 그 단계도 수행합니다. `--new-project`는 업데이트 명령이 아니며 기존 데이터를 새 프로젝트로 자동 이전하지 않습니다.
6. Vercel 웹도 사용하면 같은 버전으로 별도 재배포합니다. 로그인, 기존 경험 조회, 저장, 비서 작업 하나를 확인합니다.
...
Fetch tags, switch to a real published release tag, and run `npm start` from the repository root. It refreshes changed locked dependencies and, when the migration set changed, verifies remote history and applies only new migrations to the configured project before launching. Complete a Supabase browser login only if the CLI session expired. Creating a new project is not an upgrade. Redeploy Vercel at the same version if used, then smoke-test login, existing records, saving, and an AI task.
