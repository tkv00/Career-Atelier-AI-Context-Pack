...
구독 사용량 한도는 피하지 않는다. 한도·rate limit·reset 신호가 나타나면 실행 상태를 `waiting_for_reset`으로 저장한다. Claude가 유료 초과 사용을 허용하는 신호를 보내면 `blocked_paid_overage`로 즉시 중단한다. 재시도는 0회다 — 실패한 실행을 자동으로 다시 돌리면 쿼터가 조용히 두 배로 나간다.
...
첫 번째로 가입한 계정만 그 인스턴스의 소유자가 된다. 그 뒤로는 신규 가입 자체가 막힌다. 로그인은 매직링크 하나뿐이다.
