  ...
  return <section className="interview-page">
    ...
    <div className="interview-layout">
      <aside className="interview-library">
        ...
        <button className={scope === 'experience' ? 'active' : ''} onClick={() => setScope('experience')}><i>01</i><span><b>경험 질문</b><small>프로젝트·문제해결·실패</small></span><em>{questions.filter((item) => !item.job_post_id && item.category === 'experience').length}</em></button>
        <button className={scope === 'personality' ? 'active' : ''} onClick={() => setScope('personality')}><i>02</i><span><b>인성 질문</b><small>협업·갈등·업무 기준</small></span><em>{questions.filter((item) => !item.job_post_id && item.category === 'personality').length}</em></button>
