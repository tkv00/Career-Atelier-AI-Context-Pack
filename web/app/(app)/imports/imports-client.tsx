  ...
  return <div data-import-workspace className={styles.root}>
    ...
    <section className={styles.card}><h2>새 자료</h2>
      ...
      <form action={submit} key={mode}><input type="hidden" name="source_type" value={mode}/>
        {mode==='file'?<label>Markdown, PDF 또는 Office 파일<input name="file" type="file" accept=".md,.markdown,.pdf,.xlsx,.xls,.docx,.pptx,.csv,.html,.htm" required/><span className={styles.muted}>최대 10 MiB. 로컬 러너의 Microsoft MarkItDown이 먼저 Markdown으로 바꿔 원본 파일을 모델에 직접 보내지 않습니다. Excel은 시트·여러 줄 헤더와 저장된 수식 결과도 별도로 확인합니다.</span></label>:<label>자료 이름<input name="name" placeholder="프로젝트 회고와 경력 기록"/></label>}
