...
const body = source.split('const performCloudSave = useCallback(async () => {')[1].split('}, [essay.id, maybeSnapshot]);')[0].replace('const result: SaveDraftResult', 'const result');
...
test('recovering a stale or legacy draft preserves its base revision across another reload', async () => {
  const recovery = source.split('deviceNameRef.current = getDeviceName();\n    (async () => {')[1].split('})();')[0];
