...
export const SYSTEM_RULES = {
  ...
  jobs: [
    ...
    // company_rating은 JOBS_OUTPUT_SCHEMA(context-pack.mjs)가 필수 필드로
    // 요구한다 — 이 문장이 없으면 스키마는 지키되 값을 지어내거나(다른
    // 사이트 평점을 잡플래닛인 것처럼 적는 것) 매번 null만 채워 넣기 쉽다.
    '각 공고의 회사에 대해 잡플래닛(jobplanet.co.kr)에서 검색한 평점을 company_rating에 소수점까지(예: 3.7) 기록합니다. 잡플래닛에 없거나 확인하지 못하면 company_rating을 null로 둡니다 — 다른 사이트의 평점이나 짐작한 값을 대신 넣지 않습니다.',
    SCHEMA_COMPLIANCE,
  ].join('\n'),
