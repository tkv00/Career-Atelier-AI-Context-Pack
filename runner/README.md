...
### 2) 러너 로그인 (최초 1회)
러너는 브라우저가 없는 터미널 환경에서 실행되므로, 이메일 매직링크와 함께 발송되는 6자리 코드로 인증합니다.

```bash
npm run login
```

- 로그인 성공 시 세션 토큰은 `~/.career-atelier/session.json` (권한 0600)에 안전하게 저장됩니다.
- 토큰 갱신은 백그라운드에서 자동으로 이루어집니다.
