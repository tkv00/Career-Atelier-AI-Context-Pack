...
웹 로그인은 이메일+비밀번호지만(2026-09-03), 러너는 브라우저가 없는 백그라운드
프로세스라 비밀번호 폼을 띄울 수 없다. 그래서 이메일 OTP 메일에 함께 오는
**6자리 코드**로 로그인한다(`supabase/templates/magic_link.html`). service_role
