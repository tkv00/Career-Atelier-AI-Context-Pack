...
| `safety.mjs` | 불변 안전장치 (민감 환경변수 제거, 일일 40회 상한, 구독 인증 검사) |
...
4. 무음 대기 방지: 일일 실행 상한, 승인 해제, Supabase 조회 실패로 큐가 멈추면 러너가 원인을 한 번 출력합니다. 일일 상한으로 대기하는 작업은 6시간 유효기간 안에 로컬 자정이 지나면 재개되며, 그 전에 만료되면 수동으로 다시 실행해야 합니다.
