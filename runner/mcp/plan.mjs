...
import { refineExperiences, EXPERIENCE_POLICY_VERSION } from '../imports/metadata.mjs';

// 전송 방식과 무관한 정형 자료 처리 서비스. MCP와 직접 호출이 같은 결과를 사용한다.
export async function planImport({ source, section, only, sheet, column_map, header_row, header_rows, end_row, expected_digest }, {refine=refineExperiences}={}) {
  const started = performance.now();
  const loaded = await loadSource(source, { section, sheet, column_map, header_row, header_rows, end_row, allow_titleless_experience:true });
  ...
  const digest = createHash('sha256').update(JSON.stringify({ sourceText, section, sheet, column_map, header_row, header_rows, end_row, version:EXPERIENCE_POLICY_VERSION })).digest('hex');
  if (expected_digest && digest !== expected_digest) throw new Error('미리보기 이후 소스 또는 매핑이 변경되었습니다. 다시 미리보기 하세요.');
  const refined=await refine(filtered);
  const { rows, rejected, warnings } = buildRows(refined);
...
function normalizeJsonItems(json) {
  ...
  const items = [], skipped = [];
  list.forEach((item, i) => {
    if(item?.kind==='experience' && !item.title && item.fields && typeof item.fields==='object') {
      item={...item,title:Object.values(item.fields).find(value=>typeof value==='string'&&value.trim())||'',title_from_excerpt:true};
    }
    ...
    } else items.push({ kind: item.kind, title: item.title, fields: item.fields, line: i + 1, ...(item.title_from_excerpt?{title_from_excerpt:true}:{}) });
  });
