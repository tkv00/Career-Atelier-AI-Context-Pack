import { fieldFor, kindForSection } from './parse.mjs';

const TITLE_HEADERS = /^(title|name|제목|이름|경험명|항목명|학교명|회사명|자격증명)$/i;

// 셀 내용을 Markdown으로 재파싱하면 줄바꿈·# 문자가 다른 행의 경계가 된다.
// 표의 행 경계를 유지한 채 기존 필드 별칭만 공유한다.
export function tableToItems(headers, records, { section, column_map = {}, origin = '' } = {}) {
  const kind = kindForSection(section ?? '');
  if (!kind) throw new Error(`종류를 지정하세요: section=경험/학력/자격증 등 (${origin})`);
  const names = headers.map(h => String(h ?? '').trim());
  const used = names.filter(Boolean);
  if (new Set(used).size !== used.length) throw new Error(`중복 열 이름이 있습니다 (${origin})`);
  for (const [source, target] of Object.entries(column_map)) {
    if (!names.includes(source)) throw new Error(`매핑할 열이 없습니다: ${source}`);
    if (target !== 'title' && !fieldFor(kind, target)) throw new Error(`지원하지 않는 매핑 필드: ${target}`);
  }
  const mapped = names.map(name => column_map[name] ?? name);
  const titleColumns = mapped.map((name, i) => TITLE_HEADERS.test(name) ? i : -1).filter(i => i >= 0);
  if (titleColumns.length !== 1) throw new Error(`제목 열이 하나 필요합니다. column_map으로 title을 지정하세요 (${origin})`);
  const titleIndex = titleColumns[0];
  const fields = mapped.map((name, i) => i === titleIndex ? null : fieldFor(kind, name));
  const known = fields.filter(Boolean);
  if (new Set(known).size !== known.length) throw new Error(`여러 열이 같은 필드에 매핑됩니다 (${origin})`);
  const items = [], skipped = [], warnings = [];
  for (const [index, record] of records.entries()) {
    const line = index + 2;
    if (record.every(value => value === '' || value === null || value === undefined)) continue;
    const title = String(record[titleIndex] ?? '').trim();
    if (!title) { skipped.push({ line, origin, reason: '제목 없음' }); continue; }
    const item = { kind, title, fields: {}, line };
    for (let i = 0; i < record.length; i++) {
      if (i === titleIndex || record[i] === '' || record[i] === null || record[i] === undefined) continue;
      const value = String(record[i]);
      if (fields[i]) {
        item.fields[fields[i]] = value;
        if (fields[i] === 'secondary_major') item.fields.secondary_major_type = /복수|이중/.test(mapped[i]) ? '복수전공' : '부전공';
      } else {
        warnings.push({ line, origin, column: names[i] || `열 ${i + 1}`, reason: '지원하지 않는 열; 값은 저장하지 않음' });
      }
    }
    items.push(item);
  }
  return { items, skipped, warnings };
}
