...
경험 제목은 최대 40자, 대표 역량은 지정된 12개 중 최대 3개로 제한한다. 정책에 맞지 않는 제목·태그는 미리보기 단계부터 로컬 Codex 구독으로 정리하므로 CLI 로그인이 필요하다. 본문은 보존하며 정리 결과는 로컬 캐시로 재사용한다. 제목 없는 엑셀도 본문이 매핑되면 요약 제목을 생성한다. [역량 조사 근거와 공통 정책](../../docs/EXPERIENCE-METADATA.md).
