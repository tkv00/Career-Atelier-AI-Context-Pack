...
function htmlToMarkdown(root: HTMLElement) {
  ...
  const addBlock = (node: Node) => {
    ...
    else if (['P', 'DIV'].includes(node.tagName)) blocks.push(content);
    else blocks.push(content);
  };
  Array.from(root.childNodes).forEach(addBlock);
  return blocks.join('\n\n').replace(/\n{3,}/g, '\n\n').trim();
}

export function MarkdownCanvas({ value, onChange, onBlur, placeholder = '답변을 작성하세요…', ariaLabel = '마크다운 답변 편집기' }: MarkdownCanvasProps) {
  const editorRef = useRef<HTMLDivElement>(null);
  const latestValue = useRef(value);

  useEffect(() => {
    const editor = editorRef.current;
    if (!editor || document.activeElement === editor || value === latestValue.current) return;
  ...
  useEffect(() => {
    const editor = editorRef.current;
    if (editor && !editor.innerHTML) editor.innerHTML = markdownToHtml(value);
  }, [value]);
  ...
  function handleShortcut(event: React.KeyboardEvent<HTMLDivElement>) {
    ...
    const shortcut = shortcuts[marker];
  ...
  return (
    <div className="markdown-canvas-shell">
      <div className="markdown-toolbar" aria-label="서식 도구">
        ...
        <span>마크다운 단축키 지원 · # 제목 · - 목록 · &gt; 인용</span>
      </div>
      <div
        ref={editorRef}
